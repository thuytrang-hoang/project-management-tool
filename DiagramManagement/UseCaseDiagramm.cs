...
{
    // Form for creating and managing Use Case diagrams
    public partial class UseCaseDiagramm : Form
    {
        ...
        // Handles user clicks to create an oval
        private void oval_Click(object sender, EventArgs e)
        {
            _isInTextBoxMode = false;
            _currentCurve = null;
            _clickHandler = Oval.OvalClickHandler;
            StatusManager.Instance.SetStatus(Oval.StartMessage);

        }

        // Handles rendering the drawing and selected objects
        private void pictureBox_Paint(object sender, PaintEventArgs e)
        {
            ...
            foreach (var textBox in _textBoxes)
            {
                if (_selectedCurve == textBox)
                {
                    e.Graphics.DrawRectangle(Pens.Blue, textBox.Bounds);
                }
            }
        }
        ...
        private bool IsMouseOverCurve(Curve curve, System.Drawing.Point mousePosition)
        {
            ...
            else if (curve is Oval oval)
            {
                return IsMouseInsideOval(oval, mousePosition);
            }
        ...
        private bool IsMouseInsideOval(Oval oval, System.Drawing.Point mousePosition)
        {
            Point mousePoint = new Point(mousePosition.X, mousePosition.Y);

            // Simplified examination to determine if the mouse is within the bounds of the surrounding rectangle
            double left = oval.CenterPoint.X - oval.Width / 2;
            double right = oval.CenterPoint.X + oval.Width / 2;
            double bottom = oval.CenterPoint.Y - oval.Height / 2;
            double top = oval.CenterPoint.Y + oval.Height / 2;
        ...
        // Adds and enables interaction with a user image
        private void UserImage_Click(object sender, EventArgs e)
        {
            string imagePath = @"Images\UserImage.png";
        ...
        // Enables mouse wheel event handling for zooming
        protected override void OnLoad(EventArgs e)
        {
            base.OnLoad(e);
            pictureBox.MouseWheel += PictureBox_MouseWheel_Handler;
        }

        // Handles mouse wheel event in the picture box
        private void PictureBox_MouseWheel_Handler(object sender, MouseEventArgs e)
        {
            // Necessary to enable the mouse wheel event
